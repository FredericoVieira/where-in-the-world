...
const Home = () => {
  ...
  const { options: regionsOptions, selected: selectedRegion } =
    globalState.regions;
  const {
    fetchCountries,
    handleSelectCountry,
  ...
  useEffect(() => {
    if (countriesOptions.length > 0) {
      const countriesToShow =
        selectedRegion === "All" || selectedRegion.value === "All"
          ? countriesOptions
          : countriesOptions.filter(
              (country) => country.region === selectedRegion.value
            );
      const countriesSearch = countriesToShow.map((country) =>
        Object.assign({}, { key: country.name }, { value: country.name })
      );
      localSetState({
        ...localState,
        loaded: true,
        countriesToShow: shuffle(countriesToShow),
        countriesSearch,
      });
    }
      ...
      >
        <div className="card">
          ...
          <div className={`card-content card-content--${theme}`}>
            ...
            <p>
              <span className="card-info">Population: </span>
              {numberWithCommas(country.population)}
            </p>
            <p>
              <span className="card-info">Region: </span>
              {country.region}
            </p>
            <p>
              <span className="card-info">Capital: </span>
              {country.capital}
            </p>
          </div>
        </div>
          ...
          <div className="row search-filter">
            <div className={`col s12 m6 l5 xl5 search search--${theme}`}>
              <Search
                ...
                onSelect={(selection) =>
                  handleSearchCountry(selection, navigate)
                }
              />
            </div>
