    ...
    <nav className={`nav--${theme}`}>
      <div className="nav-wrapper">
        ...
        <div className="nav-wrapper__theme" onClick={() => toggleTheme(theme)}>
          <FontAwesomeIcon icon={theme === 'light' ? faMoonRegular : faMoonSolid} size="lg" transform={{ rotate: -25 }} />
          <span className="nav-wrapper__theme--text">{theme === 'light' ? 'Dark Mode' : 'Light Mode'}</span>
        </div>
      </div>
    </nav>
  )
